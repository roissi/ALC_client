...
const Interests = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        const interests = userChoices.filter(
          (choice) => choice.is_permanent === true,
        );
        const needs = userChoices.filter(
          (choice) => choice.is_permanent === false,
        );
        ...
        console.error(
          "Une erreur s'est produite lors de la récupération des intérêts :",
          error,
        );
      }
    };
  ...
  const toggleNeedSelection = (needId) => {
    setSelectedNeedsMap((prev) => {
      ...
      if (updatedMap[needId]) {
        setSelectedNeedsWithDuration([
          { need: parseInt(needId, 10), duration: 0 },
        ]);
      } else {
        setSelectedNeedsWithDuration([]);
      }
  ...
  const handleDurationChange = (needId, duration) => {
    ...
    setSelectedNeedsWithDuration([
      { need: parseInt(needId, 10), duration: parsedDuration },
    ]);
  };
  ...
  const validateChoices = async () => {
    if (
      ...
      selectedNeedsWithDuration.length === 0
    ) {
      toast({
        duration: 6000,
        position: "top-right",
        ...
        render: ({ onClose }) => (
          <Box color="primary" p={3} bg="error" borderRadius="md">
            <Text color="primary" fontSize="xl">
              {TOAST_MESSAGES.error.title}
            </Text>
            <Text color="primary" fontSize="lg">
              {TOAST_MESSAGES.error.description}
            </Text>
    ...
    try {
      const response = await saveUserInterests(
        selectedInterests,
      ...
      toast({
        ...
        render: ({ onClose }) => (
          <Box color="primary" p={3} bg="error" borderRadius="md">
            <Text color="primary" fontSize="xl">
              {TOAST_MESSAGES.problemInterests.title}
            </Text>
            <Text color="primary" fontSize="lg">
              {TOAST_MESSAGES.problemInterests.description}
            </Text>
            <CloseButton onClick={onClose} />
          </Box>
        ),
    ...
    <Box bg="tertiary" color="secondary" m={4} p={4} borderRadius="10px">
      ...
      >
            ...
            <Stack spacing={3}>
              {allInterests.map((interest, index) => (
                <Flex alignItems="center" key={"interest-" + index}>
                  ...
                  />
                  <Text color="secondary" fontSize="lg" ml={2}>
                    {interest.name}
                  </Text>
                </Flex>
              ))}
        ...
        <Box bg="tertiary" p={8} borderRadius="10px" border="2px solid #002136">
          ...
          <CheckboxGroup
            value={Object.keys(selectedNeedsMap).filter(
              (key) => selectedNeedsMap[key],
            )}
            onChange={(vals) => vals.forEach((val) => toggleNeedSelection(val))}
          >
            <Stack spacing={3}>
                      ...
                      <Flex direction="row" alignItems="baseline" mt={2}>
                        {choicesMade && (
                          <Text ml={2} style={{ whiteSpace: "nowrap" }}>
                            {selectedNeedsWithDuration.find(
                              (item) => item.need === need.id,
                            )?.duration || "Not set"}{" "}
                            days
                          </Text>
                        )}
                        <Input
                          ...
                          ml={4}
                          onChange={(e) =>
                            handleDurationChange(need.id, e.target.value)
                          }
                          sx={{
                            "::placeholder": {
                              color: "quinary",
        ...
        {choicesMade ? (
          <Flex direction={flexDirection} justifyContent={justifyContentValue}>
              ...
              onClick={async () => {
                ...
                toast({
                  ...
                  render: ({ onClose }) => (
                    ...
                    >
                      <Text color="primary" fontSize="xl">
                        {TOAST_MESSAGES.updated.title}
                      </Text>
                      <Text color="primary" fontSize="lg">
                        {TOAST_MESSAGES.updated.description}
                      </Text>
                      <CloseButton onClick={onClose} />
                    </Box>
                  ),
            ...
            <Button
              ...
              onClick={async () => {
                await deleteUserInterests(userId);
                setSelectedInterests([]);
                setSelectedNeedsMap({});
                setSelectedNeedsWithDuration([]);
                setChoicesMade(false);
                toast({
                  ...
                  render: ({ onClose }) => (
                    ...
                    >
                      <Text color="primary">
                        {TOAST_MESSAGES.deleted.title}
                      </Text>
                      <Text color="primary">
                        {TOAST_MESSAGES.deleted.description}
                      </Text>
                      <CloseButton onClick={onClose} />
                    </Box>
                  ),
